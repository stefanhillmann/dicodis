...
* analys what are the bes combinations
...
```{r}
  ...
  combined_f_measure <- function(criteria.data) {
    for (n in unique(criteria.data$n_gram_size)) {
      for (c in unique(criteria.data$classifier_name)) {
        for (t in unique(criteria.data$frequency_threshold)) {
          for (s in unique(criteria.data$smoothing_value)) {
            # print(sprintf('n: %s, c: %s, t: %s, s: %s', n, c, t, s))
            scenarios <- criteria.data[which(
              criteria.data$n_gram_size == n & criteria.data$classifier_name == c & criteria.data$frequency_threshold == t & criteria.data$smoothing_value == s
            ), ]
            
            result <- data.frame()
            result$tp <- sum(scenarios$true_positives)
            result$fp <- sum(scenarios$false_positives)
            result$fn <- sum(scenarios$false_negatives)
            result$f <- fmeasure(result$tp, result$fp, result$fn)
          }
        }
      }
    }
  ...
  r <- combined_f_measure(juged.data[1:4,])
  r
