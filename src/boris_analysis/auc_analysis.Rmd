...
```{r all_best_dotplots, cache=TRUE, echo=FALSE}
  # loop over cutted.all_best and print dotplot from related data in collection documents_result
  for (cn in unique(cutted.all_best$classifier_name)) {
    subplots <- c()
    for (i in 1:nrow(cutted.all_best[which(cutted.all_best$classifier_name == cn), ])) {
      ...
      title <- sprintf("D: '%s', f_min: %d,n: %d, c: '%s', l: %.2f",
                   p_row$classifier_name, p_row$frequency_threshold, p_row$n_gram_size, p_row$criteria, p_row$smoothing_value)
      
      plot <- ggplot(r, height=600, width=800, aes(x=positive_class_distance-negative_class_distance, fill=true_class)) +
        geom_histogram(position = "identity") +
        ggtitle(
          bquote(
            atop(
              list(AUC==.(round(p_row$auc, 3)), n==.(p_row$n_gram_size), lambda==.(p_row$smoothing_value), f[min]==.(p_row$smoothing_value)),
              list(D==.(p_row$classifier_name), c==.(p_row$criteria_name))
              ))) + 
    ...
    print(
        multiplot(plotlist = subplots, cols = 2)
      )
  }
```
